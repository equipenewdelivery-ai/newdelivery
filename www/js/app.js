/* SENJU THEME FOR KARENDERIA MOBILE APP V2 - 1.5.7 */
...
function onDeviceReady(){
    ...
    window.plugins.OneSignal
      .startInit("4b5786ba-a9a2-4ed5-ae56-15b37f6a4c45")
   ...
   switch(page_id)
   {
	  ...
	  case "address_form_select": 
	   ...
	   if(location_mode==1){
        ...
        placeholder("#frm_address_book_location .h_number", "Nº" );
      ...
      case "address_form_location":
        ...
        placeholder("#frm_address_form_location .h_number", "Nº" );
    ...
    ajax_request.done(function( data ) {
     	...
     	if(data.code==1){
            ...
            break;
              ...
              setValue(".h_number", data.details.data.h_number);
            ...
            if(app_settings = getAppSettings()){
              ...
              if ( data.details.page_action=="infinite_scroll") {
                if(home_all_as_list==1 && search_type=="allMerchant"){
                  MerchantList2(data.details.list, 'list_all_restaurant');
                } else {
    			  	    fillRestaurantList(data.details.list);
                }
    			    } else
...
browseLink = function(url){
	if(!empty(url)){
		if ( !krms_config.debug ){
		   browse_inapp = cordova.InAppBrowser.open(url, '_system');	
		} else {
			window.open(url);
		}
	}
		       ...
		       html+='<ons-row>';
                ...
                html+='<ons-col width="50%" style="text-align:right;">';                
		          html+='<ons-button modifier="blue_button no_shadow" onclick="fbLogin();" >';
                   ...
                   html+='<div class="col" style="width:15%;"><img class="stic-social-icon" src="lib/icons/facebook.svg" onerror="this.src=\'facebook.png\'"></div>';
		          ...
		          html+='<ons-col style="text-align:left;">';		          
		           html+='<ons-button modifier="darker_orange_button no_shadow" onclick="GLogin();" >';
			        html+='<div class="table">';
                   html+='<div class="col" style="width:15%;"><img class="stic-social-icon" src="lib/icons/google.svg" onerror="this.src=\'google.png\'"></div>';
...
sticSetCurrentAddress = function(){
  if(isLocation()){
       location_res = getLocationData();       
       $(".stic_formatted_address").html( location_res.pretty_address );
    } else {   
     if($resp = getGeocodeAddress()){
         $(".stic_formatted_address").html( $resp.formatted_address );
     }
    }
};
